...
if ( ! class_exists( 'Tribe__Events__Community__Main' ) ) {
	...
	class Tribe__Events__Community__Main {
		 ...
		 */
		const REQUIRED_TEC_VERSION = '3.12a1';
		...
		public function getDeleteFeaturedImageButton( $event = null ) {
			...
			$output = '<a rel="nofollow" class="submitdelete" href="' . esc_url( $url ) . '">Delete Image</a>';
			return $output;
		...
		if ( $action == 'delete' ) {
				switch ( $post_type ) :
					case Tribe__Events__Main::POSTTYPE:
						 return __( 'Remove an Event', 'tribe-events-community' );
					break;
					case Tribe__Events__Main::VENUE_POST_TYPE:
						return __( 'Remove a Venue', 'tribe-events-community' );
					break;
					case Tribe__Events__Main::ORGANIZER_POST_TYPE:
		...
		public function addScriptsAndStyles() {
			...
			$stylesheet_url = $this->pluginUrl . 'src/resources/css/tribe-events-community.css';
			...
			wp_enqueue_style( Tribe__Events__Main::POSTTYPE . '-community', $stylesheet_url, array(), apply_filters( 'tribe_events_community_css_version', self::VERSION ) );

			wp_enqueue_script( Tribe__Events__Main::POSTTYPE . '-community', $this->pluginUrl . 'src/resources/js/tribe-events-community.js', array( 'jquery' ), apply_filters( 'tribe_events_community_js_version', self::VERSION ) );
		...
		public function get_view_edit_links( $event_id ) {
			...
			if ( get_post_status( $event_id ) == 'publish' || current_user_can( 'edit_post', $event_id ) ) {
				$view_link = sprintf( '<a href="%s" class="view-event">%s</a>', esc_url( get_permalink( $event_id ) ), __( 'View', 'tribe-events-community' ) );
			}

			if ( current_user_can( 'edit_post', $event_id ) ) {
				$edit_link = sprintf( '<a href="%s" class="edit-event">%s</a>', esc_url( tribe_community_events_edit_event_link( $event_id ) ), __( 'Edit', 'tribe-events-community' ) );
			}
		...
		public function formCategoryDropdown( $event = null, $currently_selected = array() ) {
			...
			$shown_item_count = 9;
			?>
			<div id="event-categories" class="tribe-hide" data-more-text="<?php echo esc_attr__( 'more', 'tribe-events-community' ); ?>" data-shown-items="<?php echo absint( $shown_item_count ); ?>">
			...
			<?php
			$items = $walker->num_items;

			if ( $shown_item_count <= $walker->num_items ) {
				echo '<div class="tribe-hide"><a id="show_hidden_categories" href="">' . sprintf( __('Show all categories ( %d )', 'tribe-events-community'), $walker->num_items ) . '</a></div>';
			}
		}
		...
		public function doSettings() {
			...
			new Tribe__Events__Settings_Tab( 'community', __( 'Community', 'tribe-events-community' ), $communityTab );
		...
		public function addCommunityToolbarItems() {
			...
			if ( current_user_can( 'manage_options' ) ) {
				$wp_admin_bar->add_menu( array(
					...
					'href' => esc_url( trailingslashit( get_admin_url() ) . 'edit.php?post_type=' . Tribe__Events__Main::POSTTYPE .'&page=tribe-events-calendar&tab=community' ),
		...
		public function addLinksToPluginActions( $actions ) {
			if ( class_exists( 'Tribe__Events__Main' ) ) {
				...
				$actions['settings'] = '<a href="' . esc_url( add_query_arg( $query_args, admin_url( 'edit.php' ) ) ) . '">' . __( 'Settings', 'tribe-events-community' ) . '</a>';
			}
			return $actions;
		}
