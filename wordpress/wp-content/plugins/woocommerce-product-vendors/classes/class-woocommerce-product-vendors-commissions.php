...
class WooCommerce_Product_Vendors_Commissions {
    ...
    public function register_custom_column_headings( $defaults ) {
		$new_columns = array(
			'_commission_order' => __( 'Order' , 'wc_product_vendors' ),
	...
	public function register_custom_columns( $column_name, $id ) {
		...
		switch ( $column_name ) {
			...
			case '_commission_order':
				printf( '<a href="%1$s" title="%2%s">#%3$s</a>', get_permalink( $data ), __( 'Edit order', 'wc_product_vendors' ), $data );
			break;
			...
			foreach ( $field_data as $k => $v ) {
				...
				if( $k == '_commission_product' ) {
					...
					if( $data && strlen( $data ) > 0 ) {
						...
						$data_label = $product_name;
						$data_value = $data;
						$option = '<option value="' . $data . '" selected="selected">' . $product_name . '</option>';
					}

					$html .= '<tr valign="top"><th scope="row"><label for="' . esc_attr( $k ) . '">' . $v['name'] . '</label></th><td>';
					if ( version_compare( WC_VERSION, '2.3', '<' ) ) :
						$html .= '<select name="' . esc_attr( $k ) . '" id="' . esc_attr( $k ) . '" class="ajax_chosen_select_products_and_variations" data-placeholder="Search for product&hellip;" style="min-width:300px;">' . $option . '</select>';
					else :
						$html .= '<input type="hidden" class="wc-product-search" style="width: 100%;" name="' . esc_attr( $k ) . '" id="' . esc_attr( $k ) . '" data-placeholder="' . __( 'Search for a product&hellip;', 'wc_product_vendors' ) . '" data-action="woocommerce_json_search_products_and_variations" data-selected="' . $data_label . '" value="' . $data_value . '" />';
					endif;
					...
					if( $data && strlen( $data ) > 0 ) {
						$vendor = get_vendor( $data );
						$data_label = $vendor->title;
						$data_value = $vendor->ID;
						$option = '<option value="' . $vendor->ID . '" selected="selected">' . $vendor->title . '</option>';
					}

					$html .= '<tr valign="top"><th scope="row"><label for="' . esc_attr( $k ) . '">' . $v['name'] . '</label></th><td>';
					if ( version_compare( WC_VERSION, '2.3', '<' ) ) :
						$html .= '<select name="' . esc_attr( $k ) . '" id="' . esc_attr( $k ) . '" class="ajax_chosen_select_vendor" data-placeholder="Search for vendor&hellip;" style="min-width:300px;">' . $option . '</select>';
					else :
						$html .= '<select id="' . esc_attr( $k ) . '" name="' . esc_attr( $k ) . '" class="wc-enhanced-select">';
						$vendors = get_vendors();
						foreach( $vendors as $vendor ) {
							$html .= '<option value="' . esc_attr( $vendor->ID ) . '" ' . selected( $data_value, $vendor->ID, false ) . '>' . esc_html( $vendor->title ) . '</option>';
						}
						$html .= '</select>';
						// $html .= '<input type="hidden" class="wc-product-vendors-select" style="width: 50%;" name="' . esc_attr( $k ) . '" id="' . esc_attr( $k ) . '" data-placeholder="' . __( 'Search for a vendor&hellip;', 'wc_product_vendors' ) . '" data-action="woocommerce_json_search_vendors" data-selected="' . $data_label . '" value="' . $data_value . '" />';
					endif;
					$html .= '<p class="description">' . $v['description'] . '</p>' . "\n";
					$html .= '</td><tr/>' . "\n";
    ...
    public function mark_all_commissions_paid() {
    	...
    	if( $typenow == $this->token ) {
			...
			$redirect = esc_url_raw( add_query_arg( 'message', 'paid', $_REQUEST['_wp_http_referer'] ) );
