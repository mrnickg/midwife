...
class WC_Form_Handler {
	...
	public static function add_to_cart_action( $url = false ) {
		...
		if ( 'variable' === $add_to_cart_handler ) {
			...
			//No return value for custom handler so have to rely on wc notices
			$skip_add_to_cart_check = true;
			// Simple Products
		} else {
		...
		if ( ( $was_added_to_cart || $skip_add_to_cart_check ) && wc_notice_count( 'error' ) === 0 ) {
