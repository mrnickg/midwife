...
class WooCommerce_Product_Vendors {
	...
	public function __construct( $file ) {
        ...
        if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.4.0', '>=' ) ) {
            add_action( 'woocommerce_product_after_variable_attributes', array( $this, 'add_variation_setting' ), 10, 3 );
            add_action( 'woocommerce_process_product_meta_variable', array( $this, 'process_variation_setting' ) );
            add_action( 'woocommerce_save_product_variation', array( $this, 'save_product_commission_field_variation' ), 10, 2 );
        } else {
        ...
        if( ! vendor_access() ) {
            ...
            $html = '<div class="options_group">
                        <p class="form-field _product_vendors_commission_field">
                            ...
                            <input class="short" size="6" placeholder="0" type="number" step="any" name="_product_vendors_commission" id="_product_vendors_commission" value="' . $commission . '" />&nbsp;&nbsp;%
        ...
        $html = '<tr>
                        <td>
                            <div class="_product_vendors_commission">
                                ...
                                <input size="4" type="number" step="any" name="variable_product_vendors_commission[' . $loop . ']" id="_product_vendors_commission_' . $loop . '" value="' . $commission . '" />
                            </div>
                        </td>
                    </tr>';
    ...
    /**
     * Add vendor settings to product variations ( WC 2.4.0+ )
     * @param int $loop           Current variation loop
     * @param obj $variation_data Variation data object
     */
    ...
    /**
     * Update product variation settings ( WC 2.4.0 + )
     * @return void
     */
    public function process_variation_setting( $product_id ) {
        if ( empty( $product_id ) ) {
            return;
        }
        ...
        update_post_meta( $product_id , '_product_vendors_commission' , $commission );
    }
    ...
    public function save_product_commission_field_variation( $variation_id, $i ) {
        ...
        $commission = $_POST['variable_product_vendors_commission'][ $i ];

        update_post_meta( $variation_id, '_product_vendors_commission', $commission );
    ...
    public function vendor_details_page_save() {
        if( isset( $_POST[ 'vendor_update_nonce' ] ) && isset( $_POST['update_vendor_id'] ) ) {
            ...
            $redirect = esc_url_raw( add_query_arg( 'message', 1, $_POST['_wp_http_referer'] ) );
	...
	public function add_vendor_fields( $taxonomy ) {
        ...
        <div class="form-field">
            ...
            <?php if ( version_compare( WC_VERSION, '2.3', '<' ) ) : ?>
                <select name="vendor_data[admins][]" id="vendor_admins" class="ajax_chosen_select_customer" multiple="multiple" style="width:95%;" placeholder="<?php _e( 'Search for users', 'wc_product_vendors' ); ?>"></select><br/>
            <?php else : ?>
                <input type="hidden" class="wc-customer-search" id="vendor_admins" name="vendor_data[admins][]" data-multiple="true" data-placeholder="<?php _e( 'Search for users', 'wc_product_vendors' ); ?>" value="" data-allow_clear="true" style="max-width: 400px;" />
            <?php endif; ?>
    ...
    public function edit_vendor_fields( $vendor ) {
        ...
        $vendor_admins_array = array();
        if( isset( $vendor_data['admins'] ) && count( $vendor_data['admins'] ) > 0 ) {

        	$admins = $vendor_data['admins'];

            if( isset( $admins[0] ) && $admins[0] ) {

                if ( strpos( $admins[0], ',' ) !== false) {
                    $admins = explode( ',', $admins[0] );
                }

                foreach( $admins as $k => $user_id ) {
                    $user = get_userdata( $user_id );
                    $user_display = $user->display_name . ' (#' . $user_id . ' - ' . $user->user_email . ')';
                    $vendor_admins_array[ $user_id ] = $user_display;
                }

            }
        }

        $vendor_admins = json_encode( $vendor_admins_array );
        ...
        <tr class="form-field">
            ...
            <td>
                <?php if ( version_compare( WC_VERSION, '2.3', '<' ) ) : ?>
            	   <select name="vendor_data[admins][]" id="vendor_admins" class="ajax_chosen_select_customer" multiple="multiple" style="width:95%;" placeholder="<?php _e( 'Search for users', 'wc_product_vendors' ); ?>"><?php echo $vendor_admins; ?></select><br/>
                <?php else : ?>
                    <input type="hidden" class="wc-customer-search" id="vendor_admins" name="vendor_data[admins][]" data-multiple="true" data-placeholder="<?php _e( 'Search for users', 'wc_product_vendors' ); ?>" value="<?php echo implode( ',', array_keys( $vendor_admins_array ) ); ?>" data-selected='<?php echo esc_attr( $vendor_admins ); ?>' data-allow_clear="true" style="max-width: 400px;" />
                <?php endif; ?>
            	<span class="description"><?php _e( 'A list of users who can manage this vendor\'s products and view the sales reports.', 'wc_product_vendors' ); ?></span>
            </td>
        </tr>
    ...
    public function process_commissions( $order_id ) {
        ...
        foreach( $items as $item_id => $item ) {
            ...
            if( $product_id && $line_total ) {
                ...
                $commission = $this->record_commission( $product_id, $line_total, $discount_amount, $order_id );

                // Add commission to order item meta
                if( $commission ) {
                    wc_add_order_item_meta( $item_id, '_commission', $commission, true );
                }
     ...
     * @return void
     */
    public function cancel_commission( $order_id = 0 ) {
