...
function woocommerce_product_vendors_report_vendor_sales() {
	...
	<form method="post" action="">
		<p>
			<?php if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.3.0', '<' ) ) { ?>
				<select id="vendor" name="vendor" class="ajax_chosen_select_vendor" data-placeholder="<?php _e( 'Search for a vendor&hellip;', 'wc_product_vendors' ); ?>" style="width: 400px;"><?php echo $option; ?></select>
			<?php } else { ?>
				<input type="hidden" name="vendor" class="ajax_select_vendor" data-placeholder="<?php esc_attr_e( 'Search for a vendor&hellip;', 'wc_product_vendors' ); ?>" style="width: 400px;" />
			<?php } ?>

			<input type="submit" style="vertical-align: top;" class="button" value="<?php _e( 'Show', 'wc_product_vendors' ); ?>" />
		</p>
		...
		<script type="text/javascript">
			...
			<?php } else { ?>
				jQuery(function(){
					...
					jQuery( '.ajax_select_vendor' ).select2( {
						placeholder: "<?php esc_attr_e( 'Search for a vendor&hellip;', 'wc_product_vendors' ); ?>",
				...
				<?php
					if ( sizeof( $product_sales ) > 0 ) {
						foreach ( $product_sales as $date => $sales ) {
							...
							echo '<tr><th>' . date_i18n( 'F', strtotime( $date . '01' ) ) . '</th>
