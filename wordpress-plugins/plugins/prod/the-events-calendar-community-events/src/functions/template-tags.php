...
function tribe_community_events_get_start_date( $event_id = null ) {
	...
	$date = ( $date ) ? Tribe__Events__Date_Utils::date_only( $date ) : date_i18n( 'Y-m-d' );
...
function tribe_community_events_form_start_time_selector( $event_id = null ) {
	...
	if ( $event_id ) {
		$start_date = tribe_get_start_date( $event_id, true, Tribe__Events__Date_Utils::DBDATETIMEFORMAT );
	}
...
function tribe_community_events_form_end_time_selector( $event_id = null ) {
	...
	if ( $event_id ) {
		$end_date = tribe_get_end_date( $event_id, true, Tribe__Events__Date_Utils::DBDATETIMEFORMAT );
	}

	$end_minutes = Tribe__Events__View_Helpers::getMinuteOptions( $end_date );
	$end_hours = Tribe__Events__View_Helpers::getHourOptions( $is_all_day == 'yes' ? null : $end_date );
	$end_meridian = Tribe__Events__View_Helpers::getMeridianOptions( $end_date );
 ...
 *
 * In future releases this function can be removed and Tribe__Events__View_Helpers::is_24hr_format()
...
function tribe_community_events_use_24hr_format() {
	if ( method_exists( 'Tribe__Events__View_Helpers', 'is_24hr_format' ) ) {
		return Tribe__Events__View_Helpers::is_24hr_format();
	}
	else {
		return strstr( get_option( 'time_format', Tribe__Events__Date_Utils::TIMEFORMAT ), 'H' );
	}
}
